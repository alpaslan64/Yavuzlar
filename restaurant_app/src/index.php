...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['food_id']) && isset($_POST['quantity']) && isset($_POST['note'])) {
    $food_id = $_POST['food_id'];
    $quantity = $_POST['quantity'];
    $note = isset($_POST['note']) ? $_POST['note'] : null;

    $queryFood = "SELECT restaurant_id FROM food WHERE id = ?";
    $stmtFood = $pdo->prepare($queryFood);
    $stmtFood->execute([$food_id]);
    $foodResult = $stmtFood->fetch(PDO::FETCH_ASSOC);

    $basketEmpty = ($total_items == 0);
    $basketRestaurantId = null;

    if (!$basketEmpty) {
        $queryBasket = "SELECT food.restaurant_id FROM basket 
                         JOIN food ON basket.food_id = food.id 
                         WHERE basket.user_id = ?";
        $stmtBasket = $pdo->prepare($queryBasket);
        $stmtBasket->execute([$user_id]);
        $basketResult = $stmtBasket->fetch(PDO::FETCH_ASSOC);
        $basketRestaurantId = $basketResult['restaurant_id'];
    }

    if ($basketEmpty || ($foodResult['restaurant_id'] === $basketRestaurantId)) {
        $query = "INSERT INTO basket (user_id, food_id, note, quantity) VALUES (?, ?, ?, ?)";
        $stmt = $pdo->prepare($query);
        $stmt->execute([$user_id, $food_id, $note, $quantity]);
        header("Location: index.php"); 
        exit; 
    } else {
        $basket_error = "Sepetinizde farklı bir restorandan ürün var. Lütfen önceki ürünleri çıkarın.";
    }
}

$queryRest = "SELECT basket.*, food.name AS food_name, food.restaurant_id
               FROM basket
               JOIN food ON basket.food_id = food.id
               WHERE basket.user_id = ?";
$stmtRest = $pdo->prepare($queryRest);
$stmtRest->execute([$user_id]);
$restResult = $stmtRest->fetchAll(PDO::FETCH_ASSOC);
...
<script>
    ...
    var basketError = <?php echo json_encode($basket_error); ?>;
        if (basketError) {
            alert(basketError);
