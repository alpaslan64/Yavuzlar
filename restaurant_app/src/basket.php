...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['food_id']) && isset($_POST['quantity']) && isset($_POST['note'])) {
    ...
    $query = "INSERT INTO basket (user_id, food_id, quantity, note) VALUES (?, ?, ?, ?)";
    $stmt = $pdo->prepare($query);
...
<div class="basket-list">
    ...
    <div class="cupons">
        <form method="POST" action="">
            <input type="text" name="coupon_code" placeholder="Kupon Kodu" value="<?php echo htmlspecialchars($coupon_code); ?>">
            <button type="submit">Uygula</button>
            <?php if (!empty($coupon_code)): ?>
                <button class="remove-cupon" type="submit" name="remove_coupon">Kuponu Kaldır</button>
            <?php endif; ?>
        </form>
    ...
    <?php if(isset($cupon_success)): ?>
        ...
        <?php elseif(isset($cupon_error)): ?>
            ...
            <?php endif;?><br>
            <h3><span class="td-subtotal">Toplam Tutar: <?php echo number_format($total_price, 2); ?> TL</span></h3>
